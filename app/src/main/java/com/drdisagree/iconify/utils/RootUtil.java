...
public class RootUtil {
    ...
    public static void setPermissionsRecursively(final int permission, final String foldername) {
        ...
        if (!Shell.cmd("stat -c '%a' " + foldername).exec().getOut().contains(perm)
                || !Shell.cmd("fl=$(find '" + foldername + "' -type f -mindepth 1 -print -quit); stat -c '%a' $fl")
                        .exec().getOut().contains(perm))
