...
public class ModuleUtil {
    ...
    static void installModule() {
        ...
        Shell.cmd("printf '#!/system/bin/sh\n" + "\n" + "disable_overlays() {\n" + "for ol in $(cmd overlay list | grep -E '^.x..(IconifyComponent|com\\.android\\.shell:IconifyComponent)' | sed -E 's/^....//'); do cmd overlay disable $ol; done\n" + "}\n" + "\n" + "while [[ \"$(getprop sys.boot_completed)\" != \"1\" ]]; do\n" + "    timeout 1 getevent -l -q | grep \"KEY_VOLUMEUP\" && disable_overlays\n" + "done\n" + "\n" + "exit 0\n' > " + Resources.MODULE_DIR + "/post-fs-data.sh").exec();
