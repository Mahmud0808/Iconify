...
open class LandscapeBatteryC(private val context: Context, frameColor: Int) : BatteryDrawable() {
    ...
    private fun loadPaths() {
        val pathString = modRes.getString(R.string.config_landscapeBatteryPerimeterPathC)
        perimeterPath.set(PathParser.createPathFromPathData(pathString))
        perimeterPath.computeBounds(RectF(), true)

        val errorPathString = modRes.getString(R.string.config_landscapeBatteryErrorPerimeterPathC)
        errorPerimeterPath.set(PathParser.createPathFromPathData(errorPathString))
        errorPerimeterPath.computeBounds(RectF(), true)

        val fillMaskString = modRes.getString(R.string.config_landscapeBatteryFillMaskC)
        fillMask.set(PathParser.createPathFromPathData(fillMaskString))
        // Set the fill rect so we can calculate the fill properly
        fillMask.computeBounds(fillRect, true)

        val boltPathString = modRes.getString(R.string.config_landscapeBatteryBoltPathC)
        boltPath.set(PathParser.createPathFromPathData(boltPathString))

        val plusPathString = modRes.getString(R.string.config_landscapeBatteryPowersavePathC)
