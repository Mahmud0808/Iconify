package com.drdisagree.iconify.xposed.modules.statusbar.batterystyles
...
import com.drdisagree.iconify.xposed.modules.extras.utils.misc.AlphaRefreshedPaint
...
open class CircleBattery(private val mContext: Context, frameColor: Int) : BatteryDrawable() {
    ...
    fun setMeterStyle(batteryStyle: Int) {
        mFramePaint.pathEffect =
            if (batteryStyle == BATTERY_STYLE_DOTTED_CIRCLE) DASH_PATH_EFFECT else null
        mBatteryPaint.pathEffect =
            if (batteryStyle == BATTERY_STYLE_DOTTED_CIRCLE) DASH_PATH_EFFECT else null
    }
    ...
    override fun setColors(fgColor: Int, bgColor: Int, singleToneColor: Int) {
        mFGColor = fgColor
        mBoltPaint.color = mFGColor
        mFramePaint.color = bgColor
        mFramePaint.alpha = 70
        mTextPaint.color = mFGColor
        initColors()
        invalidateSelf()
    }
    ...
    private fun initColors() {
        customBlendColor = try {
            Xprefs.getBoolean(XposedKey.CUSTOM_BATTERY_BLEND_COLOR)
        } catch (ignored: Throwable) {
            RPrefs.getBoolean(XposedKey.CUSTOM_BATTERY_BLEND_COLOR)
        }
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (charging && batteryLevel < 100) {
            ...
            mBoltPaint.alpha = (mBoltAlphaAnimator.animatedValue as Int * mAlphaPct).roundToInt()
        ...
        if (!charging && batteryLevel < 100 && mShowPercentage) {
            ...
            val textHeight = -mTextPaint.fontMetrics.ascent
    ...
    override fun setAlpha(alpha: Int) {
        ...
        mFramePaint.alpha = (70 * alpha / 255f).roundToInt()
        mTextPaint.alpha = alpha
        mBatteryPaint.alpha = alpha
    }
    ...
    private fun updateSize() {
        ...
        mDiameter = bounds.bottom - bounds.top
        ...
        val scaleF =
            (bounds.height() - strokeWidth * 2) * .8f / pathBounds.height() //scale comparing to 80% of icon's inner space
        ...
        //moving it to center
        mBoltPath!!.offset(
            bounds.centerX() - pathBounds.centerX(),
            bounds.centerY() - pathBounds.centerY()
        )
    }
    ...
    init {
        ...
        mBoltAlphaAnimator.duration = 2000
