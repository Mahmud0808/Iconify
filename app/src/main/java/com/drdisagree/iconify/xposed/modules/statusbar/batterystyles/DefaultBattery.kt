...
open class DefaultBattery(private val context: Context, frameColor: Int) : BatteryDrawable() {
    ...
    private fun updateSize() {
        ...
        perimeterPath.computeBounds(pathBounds, true)

        val centerX = (WIDTH - pathBounds.width()) / 2f - pathBounds.left
        val centerY = (HEIGHT - pathBounds.height()) / 2f - pathBounds.top
        ...
        perimeterPath.transform(combinedMatrix, scaledPerimeter)
        errorPerimeterPath.transform(combinedMatrix, scaledErrorPerimeter)
        fillMask.transform(combinedMatrix, scaledFill)
        scaledFill.computeBounds(fillRect, true)
        boltPath.transform(combinedMatrix, scaledBolt)
        plusPath.transform(combinedMatrix, scaledPlus)
